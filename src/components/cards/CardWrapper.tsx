...
const CardWrapper = ({ body, footer, headerIcon: Icon, headerText, subHeaderText, action, actionLabel }: Props) => {
  return (
    <div className='flex items-center justify-center vertical-center h-verticalCenter'>
      <Card className='w-2/5 mx-auto p-5'>
        <CardHeader className='flex flex-col items-center justify-center'>
          <div className='flex flex-col gap-2 items-center text-jcb'>
        ...
        <CardFooter className='flex flex-col justify-center'>
          {action && (
            <Button onClick={action} fullWidth className='bg-jcb font-LatoBold'>
              {actionLabel}
            </Button>
          )}
          {footer && <>{footer}</>}
